...
- [[questions/z-image-figure10-architecture-walkthrough]] — Z-Image 図10 の読み解き（入力・処理・出力、**連結 self-attention は cross-attention を部分行列として含む**、編集時の二重符号化、$t=0$ がノイズ・$t=1$ が画像という向き）
- [[questions/flux2-klein-lora-training-and-composition]] — FLUX.2 klein 4B の LoRA 学習（base か蒸留版か）と前景×背景の合成（**推論ステップ数が手法を決める**：静的マージ vs 実行時スケジューラ、共有変調 4 行列を標的から外す）
