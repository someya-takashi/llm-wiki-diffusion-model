...
## [2026-08-26] schema-update | ingest skill：コードリポジトリを正式な原典系統に昇格

- 変更対象: `.claude/skills/ingest/SKILL.md`
- 動機: ユーザーの指示「今後は code_analysis に置いたリポジトリ（コード）も一次資料として取り込みたい」。前日の FLUX.2 ingest（[[summaries/2025-flux2]]）で規約を暫定的に追加していたが、**「画像の扱い」の節の中に「ケース D」として置かれており、取り込みパイプライン全体を扱う内容としては位置が不適切だった**。独立した節へ昇格させ、標準フローにも分岐を通した。
- 変更点:
  1. **frontmatter の `description` を更新** — `code_analysis/` 配下にリポジトリを置いて「解析して」と指示された場合も skill が起動するようにした。翻訳が「コードでは作らない」ことも明記。
  2. **冒頭に「原典には 2 系統ある」の一文を追加** — 論文・記事（`raw/`）とリファレンス実装（`code_analysis/`）。標準フローは共通だが翻訳・画像・コミットの扱いが変わる、と最初に宣言する。
  3. **標準フローに分岐を入れた** — step 1（読解: コードは読み方が変わる）、step 3（翻訳: `source_kind: code` では丸ごと飛ばす）、step 10（コミット: `code_analysis/` は gitignore 済みなので `git add -A` を使わず**ファイル指定で add** する）。
  4. **「コードリポジトリ（`code_analysis/`）の取り込み」を独立した節として新設**（「要約ページの仕様」と「画像の扱い」の間）。「ケース D」に散らばっていた内容を移し、FLUX.2 の実作業で得た知見を追加した。
  5. **「原典の取得経路」を 3 ケース → 4 ケース**に更新し、ケース D をリポジトリの clone 先として位置づけた。「画像の扱い」側のケース D は**画像を取り込まない**旨だけを残すポインタに縮約。
- 新設した節に書いた内容（FLUX.2 の実作業から得たもの）:
  - **読解は領域分割で並列に行う**。拡散モデルの実装なら (1) モデル定義 (2) サンプリングと CLI (3) エンコーダと設定レジストリ、の 3 分割がよく効く。subagent を使うなら「識別子を verbatim で報告せよ／行番号を添えよ／数値を丸めるな」と明示する。
  - **主要な構成値は必ず自分で `grep`／`sed` して裏取りする。** subagent の報告をそのまま転記しない。設定 dataclass は十数行なので直接読める。
  - **真実は設定 dataclass / レジストリにある。変種ごとに別クラスになっていることが多く、変種を取り違えやすい。**
  - **README・モデルカード・外部解説サイトと食い違ったらコードを正とし、食い違い自体を「限界・批判的視点」に記録する。**
  - **既存ページの訂正を必ず点検する**——コードの ingest は過去に Web 二次情報から書いた記述を覆すことがある。誤記の置換で済ませず「訂正の記録」節を設け、**何が誤っていたか・出典は何だったか・結論は変わるのか**を残す。
  - **要約ページの構成をコード版として別に定義**した。論文向けの「実験結果と知見」に対応物がないため、`構成値` / `入力・処理・出力` / `条件付けの経路` / `推論` / `実装から読める設計判断` / **`コードからは分からないこと`** に置き換える。
