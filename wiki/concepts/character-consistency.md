...
## 隣接：同一性を保ちながら「見た目」を変える

本ページは「同じ被写体を、ターンをまたいで同じに保つ」ことを扱うが、**意図的に見た目だけを変えたい**場面もある——同じキャラクターを別の画風で描く、といった要求である。ここで必要になるのは、**同一性（content）と様式（style）を切り分ける**能力であり、[[style-content-disentanglement]] がこれを扱う。

両者は同じトレードオフの両端にある。visual drift は「変えるつもりがないのに変わってしまう」失敗であり、スタイル転送の失敗は「変えたいのに変わらない（コンテンツへの過学習）」あるいは「変えすぎて別物になる」失敗である。**何を不変量とみなすか**の設定が違うだけで、扱っている量は同じである。
