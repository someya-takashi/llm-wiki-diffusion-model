...
## 4 方向のどれでもない小細工 — パディングで整列を買う（2026-08-26 追記）

本ページは効率化を「近似・疎化・実装最適化・トークン削減」の 4 方向で整理してきた。Z-Image のリファレンス実装（[[summaries/2025-z-image]]）には、**そのどれにも入らないが実測では効く**種類の工夫がある——**トークン数をあえて増やす**。

```python
SEQ_MULTI_OF = 32                                   # src/config/model.py L4
image_padding_len = (-image_ori_len) % SEQ_MULTI_OF # transformer.py L427
```

画像トークン列もテキストトークン列も、**32 の倍数になるまでパディングされる**。詰めるのはゼロではなく**学習済みのパラメータ**（`x_pad_token` / `cap_pad_token`, L328–329）で、位置符号化の座標は $(0,0,0)$——本ページの姉妹ページ [[position-embedding]] に書いたとおり、**位置 0 はパディング専用に予約されている**。そしてこれらのパッドは attention マスクで除外されず、**通常のトークンとして attention に参加する**（L517–519 が `seq_len`（パディング込み）まで 1 を立てる）。

つまり**計算量は増えている**。1024×1024 なら画像は 4096 トークンで割り切れるが、非正方形の解像度や短いキャプションでは最大 31 トークン分の無駄が出る。それでも整列させるのは、**FlashAttention 系カーネルとテンソルコアが系列長の整列を前提に速い**からである。「$O(N^2)$ の $N$ を減らす」という本ページの主眼と逆向きに、**$N$ を少し増やして定数項を大きく削る**——実装最適化の道（FlashAttention）の延長線上にあるが、モデル側が形を合わせに行っている点が違う。

**「ゼロ埋めではなく学習済みトークン」という選択も記録に値する。** ゼロベクトルを attention に混ぜると softmax の分母を歪めるが、学習可能なパッドトークンなら**モデルが「これは無視してよい」を学べる**。マスクを正確に扱う代わりにパラメータで解く、という割り切りである。

**backend の並び**も見える。FlashAttention-2 / FlashAttention-3 / それぞれの varlen 版 / **`mps_flash`（Apple Silicon 向け）** / PyTorch SDPA / 素の math を実行時に切り替える（`src/utils/attention.py`）。デバイス選択は cuda → TPU（torch_xla）→ mps → cpu の順に落ちる（`inference.py` L32–49）。**「コンシューマ級ハードウェアで動く」という主張の実体は、モデルを小さくしたこととカーネルの移植性の両方にある**。

なお `n_kv_heads == n_heads == 30` で、**GQA（Grouped-Query Attention）は使われていない**。拡散モデルは自己回帰と違って KV キャッシュを持ち越さないため、GQA の主な旨味（キャッシュのメモリ削減）が効かない——設定項目としては存在するが有効化されていない。
