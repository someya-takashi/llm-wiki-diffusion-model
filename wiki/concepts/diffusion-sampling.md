...
## 応用：ステップの役割分担を合成のスケジュールに使う

本ページは「何ステップで、どうたどるか」を設計してきたが、**どのステップで何が決まるか**という性質は、サンプリング以外の設計にも使える。

**K-LoRA**（[[summaries/2025-k-lora]]）は、被写体 LoRA と画風 LoRA を合成する際にこれを利用する。著者らの観察は明快で、**初期のタイムステップにスタイル LoRA を当てると物体の再構成そのものを壊す**が、後期に当てるとスタイルだけが乗る。コンテンツ LoRA は逆で、初期に当てた方が明確に良い。そこで時間依存のスケール係数 $S=\alpha\, t/T+\beta$ で選択を傾け、**前半は被写体、後半は画風**が優勢になるようにする。

これは本ページで繰り返し現れた「初期ステップが大域的な構図を決め、後期が細部を詰める」という性質の、**推論時の資源配分への応用**である。同じ性質は [[noise-schedule]] では学習側（SFT で後期ステップに重みを置く）、[[inference-caching]] では計算の間引き（後期は CFG の 2 回評価の意味が薄れる）に使われており、**「ステップごとに役割が違う」という 1 つの観察が 3 通りに活用されている**ことになる。詳細は [[lora-merging]]。
...
- [[summaries/2025-k-lora]] — K-LoRA（ステップの役割分担を LoRA 合成のスケジュールに応用。初期＝被写体・後期＝画風）
