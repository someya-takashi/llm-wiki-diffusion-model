...
- [[stochastic-interpolants]]：確率的補間（[[summaries/2024-stochastic-interpolants]]）は DDPM のノイズ除去を、補間のノイズ除去器 $\eta_z=\mathbb{E}(z|x_t)$ とスコア $s=-\gamma^{-1}\eta_z$ の関係として一般化し、拡散（SDE）とフロー（ODE）を同じ密度の異なる実現として統一する。DDPM は片側補間（基底がガウス）の特別な場合にあたる。
- [[noise-schedule]]：DDPM の線形 β スケジュールと損失重みは、ノイズスケジュール設計の出発点。EDM（[[summaries/2022-edm]]）はこれを精緻化し、学習ノイズ分布を対数正規（$P_{\rm mean},P_{\rm std}$）にして中間ノイズレベルに学習を集中、損失重み $\lambda(\sigma)=1/c_{\rm out}^2$ で全 σ の実効損失を均一化した。
- [[diffusion-model-architecture]]：ε 予測ネットワークの設計。ADM の改良 U-Net・AdaGN が標準を確立し、DiT が Transformer 化した。いずれも DDPM の ε／Σ 予測・$L_\text{simple}$ という数学はそのままで、バックボーンだけを差し替えている。
...
- [[summaries/2022-edm]] — EDM（対数正規ノイズ分布・損失重み $\lambda(\sigma)=1/c_{\rm out}^2$ で学習目的を精緻化）
- [[summaries/2025-flow-matching-diffusion-intro]] — Flow Matching と拡散モデル入門（MIT 6.S184 講義ノート。denoising diffusion＝ガウス確率パスの拡散モデルとして位置づけ、順過程・時間反転・ELBO の文献ガイドを提供）
