...
## 「微調整のしやすさ」を公式が表で示す（Z-Image, 2026-08-26 追記）

前節では蒸留モデルの上で学習するための回避策（assistant LoRA）を記録した。蒸留が事後の適応に何を課すのかについて、**Z-Image のリポジトリは公式の表で立場を明示している**（`README.md` の Model Zoo・[[summaries/2025-z-image]]）。

| モデル | 事前学習 | SFT | RL | ステップ | CFG | 多様性 | **微調整のしやすさ** |
| --- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
**蒸留版だけが「N/A」——すなわち微調整の対象として想定されていない。** 本ページが記録してきた「蒸留は多様性を狭める」（OneIG Diversity がベース 0.194 → Turbo 0.139）が、そのまま「Diversity: Low」として同じ表に並んでいる点も分かりやすい。

**同時に、この表は蒸留パイプラインの構造も見せている。** Omni-Base（事前学習のみ）→ Z-Image（＋SFT）→ Z-Image-Turbo（＋RL・蒸留）という直列で、**多様性は右へ行くほど単調に下がり、微調整のしやすさも下がる**。本ページが扱ってきた「蒸留はモデルを作り替える」という主張の、最も素直な可視化になっている。

にもかかわらず、**[[concepts/ai-toolkit]] には Z-Image 用の assistant LoRA（蒸留打ち消しアダプタ、multiplier $=-1$）が用意されている**。公式が「N/A」と書いたモデルで学習したい需要が実在し、ツール側がそれに応えている——**公式の推奨と現場の運用が食い違っている**例として記録に値する。前節で述べたとおり、この回避策が打ち消すのは軌道の歪みであって、失われた分布の広がり（Diversity 0.194 → 0.139）は戻らない。非蒸留の Z-Image が 2026-01-27 に公開された以上、**アダプタを噛ませる理由は「速い推論のまま学習したい」以外にはほぼ残らない。**
