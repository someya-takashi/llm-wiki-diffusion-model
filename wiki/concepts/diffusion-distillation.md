...
| **蒸留**（本ページ） | **モデルそのもの**（重み） | 必要（生徒を学習する） | DMD・consistency models・progressive distillation |
...
- **教師を超えられない**（通常は）。蒸留は教師の能力を圧縮する枠組みであり、生徒の上限は教師である。
...
> 未取り込みの主要原典：Progressive Distillation（Salimans & Ho 2022）、Consistency Models（Song ら 2023）、DMD 原典（Yin ら 2024）、Adversarial Diffusion Distillation（Sauer ら 2024）。今後の ingest で本ページへ追記する。
