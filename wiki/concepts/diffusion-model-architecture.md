...
## 「条件付けを安くする」を数値で見る — Z-Image の実装（2026-08-26 追記）

本ページは adaLN-Zero（DiT）→ adaLN の MLP を全層共有（Wan）→ 変調行列を DiT 全体で 4 つに畳む（FLUX.2）という流れで「**条件付け機構は思ったより安く済ませてよい**」を追ってきた。Z-Image のリファレンス実装（[[summaries/2025-z-image]]）は、この主張に**初めて具体的な金額をつける**。

報告書は「射影を低ランクの対に分解し、共有の下方射影＋層ごとの上方射影にする」としか書かない。コードを読むと、**共有下方射影の出力次元は 256 である**（`ADALN_EMBED_DIM`）。

```
t ──► t_embedder ──► ℝ²⁵⁶ ──┬─► layers.0.adaLN_modulation.0 : Linear(256 → 4×3840)
      （全層で 1 回）        ├─► layers.1.adaLN_modulation.0 : Linear(256 → 4×3840)
                            └─► …（30 層）
```

素朴に $\text{dim}\to4\cdot\text{dim}$（$3840\to15360$）とすると 30 層で **17.70 億パラメータ**。実際は **1.18 億**。**差は約 16.5 億**で、素朴実装なら 6.15B のモデルが約 7.8B に膨らむ——**モデル全体の 2 割強が変調機構に消えていたはず**だった、ということになる。DiT の adaLN-Zero が「パラメータの相当部分を変調に使う」設計であったことを思い出すと、この節約は小さくない。

**加えて、変調は 4 分割で shift 項がない。**

| | 分割数 | 内訳 |
| --- | --- | --- |
| DiT の adaLN-Zero | 6 | shift / scale / gate ×（attn, MLP） |
| **Z-Image** | **4** | **scale / gate ×（attn, MLP）** |

さらにゲートは `tanh()` を通り、スケールは `1.0 + ·` の形をとる（`transformer.py` L180–184）。**ゼロ初期化ではなく有界化**である——ゲートが構造的に $(-1,1)$ を出られないので、学習中に特定の層が暴走することが原理的に起きない。adaLN-Zero が「初期値をゼロに置く」ことで恒等写像から出発したのに対し、Z-Image は「**値域を縛る**」ことで同じ安定性を常時保証している。設計思想としては別物である。
...
| `context_refiner` | 2 | **なし** | テキストトークンのみ |
| `layers` | 30 | あり | 連結された統一系列 |

wiki が「30 層」と呼んできたのは統一バックボーンだけで、**総ブロック数は 34**。そして**テキスト側の入口プロセッサはタイムステップ変調を受けない**——テキスト表現はノイズ除去の進行度に依存しないのだから自然だが、報告書はこの非対称に触れていない。連結順も **`[画像, テキスト]`** で、FLUX 系の `[テキスト, 画像]` と逆である（L552）。

細部を 2 つ。FFN は SwiGLU で中間次元 $\lfloor 3840/3\times 8\rfloor = 10240$、全線形層に bias がない。そして `n_kv_heads == n_heads == 30` で、**GQA（Grouped-Query Attention, K/V のヘッドを減らして KV キャッシュを縮める手法）は使っていない**——拡散モデルは自己回帰と違って KV キャッシュを持ち越さないので、GQA の主な旨味がない。パラメータが用意されているだけで有効化されていない、という形で残っている。

**訂正**: 原典 表 2 の「注意ヘッド数 32」は誤りで、実装は **30 ヘッド・head_dim 128** である（`assert head_dim == sum(axes_dims)` が 32 ヘッドを排除する）。経緯は [[summaries/2025-z-image]] の「訂正の記録」に残した。
