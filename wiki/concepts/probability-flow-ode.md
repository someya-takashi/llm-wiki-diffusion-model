...
- [[stochastic-interpolants]]：確率的補間（[[summaries/2024-stochastic-interpolants]]）は、補間密度が満たす 1 階輸送方程式から確率フロー ODE を、Fokker-Planck 方程式から SDE を導く。確率フロー ODE（決定論）と逆時間 SDE（確率）を「同じ密度の異なる実現」として明示的に統一し、ODE は厳密尤度・適応積分の利点を、SDE は近似誤差への頑健性を持つという使い分けの理論基盤を与える。
- [[noise-schedule]]：EDM（[[summaries/2022-edm]]）は確率フロー ODE を $\sigma(t)$・$s(t)$ で直接定義し、**σ(t)=t, s(t)=1** を採ると σ と時刻が交換可能になり ODE が $\mathrm{d}\boldsymbol{x}/\mathrm{d}t=(\boldsymbol{x}-D(\boldsymbol{x};t))/t$ に簡約、解軌道がほぼ直線化する。これと Heun 2 次法で確率フロー ODE を少 NFE で解く実用化を達成した。
...
- [[summaries/2024-stochastic-interpolants]] — Stochastic Interpolants（輸送方程式→ODE、FPE→SDE で決定論的生成と確率的生成を統一）
- [[summaries/2022-edm]] — EDM（σ(t)=t で σ と時刻を交換可能にし軌道を直線化、Heun で少 NFE の ODE サンプリング）
- [[summaries/2025-flow-matching-diffusion-intro]] — Flow Matching と拡散モデル入門（MIT 6.S184 講義ノート。flow＝ODE 軌道、marginal vector field と連続の方程式、確率フロー ODE を初学者向けに導く）
