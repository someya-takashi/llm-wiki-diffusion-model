...
- **ツール・ライブラリの専用ページも原則作らない**（学習・推論フレームワーク等は該当する概念ページや `summaries/` 内で言及する）。ただし**ユーザーの明示的な判断による例外を認める**。現時点の例外は `concepts/ai-toolkit`（2026-08-26, ユーザー指示）のみ。例外ページには冒頭に「この規約に対する例外である」旨を明記し、lint 時に規約違反として検出されないようにする。
